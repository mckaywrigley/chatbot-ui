...
export const Chat = memo(({stopConversationRef}: Props) => {
    ...
    const handleAudioFileUpload = useCallback(
        async (file: File) => {
            if (selectedConversation && file && file.type.toLowerCase().startsWith("audio")) {
                homeDispatch({field: 'loading', value: true});
                homeDispatch({field: 'messageIsStreaming', value: true});
                let message: Message = {
                    role: "assistant",
                    content: `Ich habe die Datei ${file.name} von dir erhalten. Ich werde sie für dich transkribieren und schicke dir eine Email mit dem Ergbenis.`
                }
                let updatedConversation: Conversation;
                updatedConversation = {
                    ...selectedConversation,
                ...
                try {
                    const blobService = new BlobServiceClient(
                        `https://assistentenspeicher.blob.core.windows.net/?sv=2022-11-02&ss=bfqt&srt=sco&sp=rwdlacupiytfx&se=2024-05-16T19:41:11Z&st=2023-10-15T11:41:11Z&spr=https&sig=l1%2F34ZL9jZFYWqxWedZbSMydrrNvqkpyXI5%2B9KFreW4%3D
`
                    );
                    const containerClient = blobService.getContainerClient("audiofiles");
            ...
            {!(apiKey || serverSideApiKeyIsSet) ? (
                <div className="mx-auto flex h-full w-[300px] flex-col justify-center space-y-6 sm:w-[600px]">
                    <div className="text-center text-4xl font-bold text-black dark:text-white">
                        {t("Hello, I am your personal AI Assistant")}
                    </div>
                    <div className="text-center text-lg text-black dark:text-white">
                        <div className="mb-8">
                            {t("I can assist you with different tasks like")}
                        </div>
                        <div className="mb-8">
                            <ul>
                                <li>{t("Audio file transcription")}</li>
                                <li>{t("Searching for information in your digital documents")}</li>
                                <li>{t("Drafting emails")}</li>
                            </ul>
                        </div>
                    </div>
                </div>
